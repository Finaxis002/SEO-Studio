...
api.get = (path, opts) =>
  api(path.startsWith("/api") ? path.slice(4) : path, { ...opts, method: "GET" });
api.post = (path, body, opts) =>
  api(path.startsWith("/api") ? path.slice(4) : path, {
    ...opts,
    method: "POST",
    body,
  });
...
export function getVinimayUrl(path = "", customBase) {
  const base = (
    customBase ||
...
export function getVinimayBlogUrl(slug, customBase) {
  if (!slug) return "#";
  const cleanSlug = String(slug).replace(/^\/?(blogs\/)?/, "");
  return getVinimayUrl(`/blogs/${encodeURIComponent(cleanSlug)}`, customBase);
}
