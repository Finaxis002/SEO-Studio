...
export async function getGoogleIndexing(blogs = []) {
  const siteUrl = (process.env.GSC_SITE_URL || "").trim();
  const vinimayBase = (
  ...
  let gscPageMap = new Map();

  if (siteUrl && process.env.GOOGLE_SERVICE_ACCOUNT_JSON) {
    try {
      const auth = await authClient();
      const search = google.searchconsole({ version: "v1", auth });
      const dates = rangeDates(90);
      const rows = await gscQuery(search, siteUrl, dates, ["page"], 500);
      rows.forEach((row) => {
        const fullUrl = row.keys?.[0] || "";
        gscPageMap.set(fullUrl, {
          impressions: number(row.impressions),
          clicks: number(row.clicks),
          position: Number((row.position || 0).toFixed(1)),
          ctr: Number(((row.ctr || 0) * 100).toFixed(1)),
      ...
      console.warn("getGoogleIndexing GSC query failed:", e.message);
    }
  }

  const items = blogs.map((b) => {
    const slug = b.slug || "";
    const fullBlogUrl = `${vinimayBase}/blogs/${encodeURIComponent(slug)}`;
    
    // Check if GSC has tracked this page directly or by path
    let gscData = gscPageMap.get(fullBlogUrl);
    if (!gscData) {
      for (const [key, val] of gscPageMap.entries()) {
    ...
    const isIndexed = !!gscData && gscData.impressions > 0;

    return {
      id: b.id,
      title: b.title,
      slug,
      featuredImage: b.featuredImage?.url || "",
      fullUrl: fullBlogUrl,
  ...
  const indexedCount = items.filter((i) => i.isIndexed).length;
  const pendingCount = items.length - indexedCount;
  ...
  return {
    summary: {
      ...
      rate: items.length ? Math.round((indexedCount / items.length) * 100) : 0,
      siteUrl,
    },
    items,
  };
